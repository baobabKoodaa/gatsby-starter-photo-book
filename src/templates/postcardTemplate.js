...
class PostcardTemplate extends React.Component {
  ...
  componentWillMount() {
      if (typeof window !== 'undefined') {
       window.scrollTo(0,100000); /* Hack to hide Chrome address bar on Android to provide more screen real estate for images. */
    }
  }
        ...
        {globalState => (
            <>
                ...
                <Link to={`/images/${c.prevId}`}>
                      <span style={{ position: "fixed", height: "100vh", width: "50vw", left: "0px", zIndex: 2 }}></span>
                </Link>
                <Link to={`/images/${c.nextId}`}>
                      <span style={{ position: "fixed", height: "100vh", width: "50vw", right: "0px", zIndex: 2 }}></span>
                </Link>
